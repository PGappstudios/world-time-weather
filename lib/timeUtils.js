...
const preloadCommonCities = () => {
  ...
  commonCities.forEach(cityName => {
    ...
    if (timezone) {
      ...
      timezoneCache.set(cityName.toLowerCase(), {
        timezone,
        datetime: now.format(),
...
// Initialize cache with common cities
preloadCommonCities();
...
export const getTimezoneData = async (cityName) => {
  try {
    ...
    try {
      // Get data from WorldTimeAPI without timeout
      ...
      // Cache the result
      timezoneCache.set(cacheKey, data);
      return data;

    } catch (apiError) {
      // If WorldTimeAPI fails, use Moment.js data immediately
      const now = moment.tz(timezone);
      const data = {
        timezone,
        datetime: now.format(),
    ...
    // Return basic timezone data even if everything else fails
    const timezone = findTimezoneForCity(cityName);
    if (timezone) {
      const now = moment.tz(timezone);
      return {
...
export const findTimezoneForCity = (cityName) => {
  ...
  // Try to find by exact city name in Moment.js zones
